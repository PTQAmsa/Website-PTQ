...
export default function MIResultModal({ answers, onClose }: Props) {
  ...
  // Hitung skor
  const scores = miQuestions
    .map((q) => ({ type: q.type, score: answers[q.id] || 0 }))
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Kirim ke API untuk arsip spreadsheet — fire and forget dari sisi UX
    fetch('/api/submit-mi', {
      method: 'POST',
    ...
    }).catch((err) => console.error('[MIResultModal] submit error:', err));

    // Langsung tampilkan hasil tanpa tunggu response
    setLoading(false);
    setStep('result');
  };
